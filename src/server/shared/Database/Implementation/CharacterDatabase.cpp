...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_DEL_ARENA_TEAM_BY_ID, "DELETE FROM arena_team WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ARENA_TEAM_ID_BY_PLAYER_GUID, "UPDATE arena_team_member SET arenaTeamId = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_ARENA_TEAM_ID_BY_CAPTAIN_GUID, "SELECT arena_team.arenateamid FROM arena_team WHERE captainGuid = ? AND type = ? LIMIT 1", CONNECTION_SYNCH);
